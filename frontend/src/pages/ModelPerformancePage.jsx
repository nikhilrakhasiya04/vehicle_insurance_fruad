...
} from "@/components/performance/ModelMetricsChart";
...
export default function ModelPerformancePage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setData(payload);
      setSelected(payload?.best_model);
    } catch (err) {
  ...
  if (!data?.models)
    return <ErrorMessage message="No model data. Please run train.py first." />;

  const modelList = Object.entries(data.models).map(([name, m]) => ({
    name,
    ...m,
    isSelected: name === (data.best_model || data.best_model_name),
  }));
  const selectedModel = modelList.find(
    (m) => m.name === (selected || data.best_model)
  );
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="ml-auto text-right">
            ...
            <p className="text-sm text-gray-300">
              Train: {data.train_size?.toLocaleString()} | Test:{" "}
              {data.test_size?.toLocaleString()}
            </p>
            <p className="text-xs text-gray-500 mt-0.5">
              Fraud rate:{" "}
              {((data.class_distribution?.fraud_rate || 0) * 100).toFixed(1)}%
            </p>
          </div>
        </div>
      </div>
        ...
        {modelList.map((m) => (
          <button
            ...
            className={`text-left p-4 rounded-xl border transition-all ${
              selected === m.name
                ? "border-primary-600 bg-primary-900/20 ring-1 ring-primary-500"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="card">
          <p className="text-sm font-semibold text-gray-200 mb-3">
            F1-Score{" "}
            <span className="text-xs text-gray-500">(green = selected)</span>
          </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="card">
            <p className="text-xs text-gray-500 mb-3 font-medium">
              ...
              <span className="text-blue-400">{selectedModel.name}</span>
            </p>
            <div className="space-y-2">
              ...
              <div className="bg-gray-800/60 rounded-lg p-3">
                ...
                <p className="text-2xl font-bold text-primary-400">
                  {(
                    (selectedModel.classification_report?.accuracy ||
                      selectedModel.test_accuracy ||
                      0) * 100
                  ).toFixed(1)}
                  %
                </p>
