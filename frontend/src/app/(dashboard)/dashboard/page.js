...
export default function DashboardPage() {
  ...
  const load = async () => {
    ...
    try { const res = await fetchStats(); setStats(res.data); }
        ...
        <div className="card">
          ...
          <div className="space-y-3">
            {[
              ...
              { label:"Fraud Detected",    value: s.totalFraud    ?? 0, pct: s.total ? (s.totalFraud    / s.total) * 100 : 0,                   color:"bg-red-500"   },
              { label:"Legitimate",        value: s.totalNotFraud ?? 0, pct: s.total ? (s.totalNotFraud / s.total) * 100 : 0,                   color:"bg-green-500" },
          ...
          {s.total === 0 && (
            <p className="text-xs text-gray-600 mt-6 text-center">Submit a claim on the Fraud Prediction page to see stats here.</p>
          )}
        </div>
      </div>
    </div>
